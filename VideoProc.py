...
APP_DIR = LOCAL_APP_DATA / "VideoProc"
...
class DependencyManager:
    ...
    def validate_installation(self):
        ...
        if not os.path.exists(ffmpeg_path) or not os.path.exists(ffprobe_path):
            return False
        try:
            res = subprocess.run([ffmpeg_path, "-version"], capture_output=True, text=True, timeout=10,
                                 creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
            return res.returncode == 0 and "ffmpeg version" in res.stdout.lower()
        except Exception:
            return False

    def install_ffmpeg(self, progress_callback=None):
        self.log("INFO", "Missing dependency detected. Downloading FFmpeg...")
        if os.path.exists(FFMPEG_DIR):
            try: shutil.rmtree(FFMPEG_DIR)
            except Exception as e: self.log("ERROR", f"Could not clear old ffmpeg directory: {e}")
        os.makedirs(FFMPEG_DIR, exist_ok=True)
        zip_path = APP_DIR / "ffmpeg_download.zip"
        
        try:
            with urllib.request.urlopen(self.FFMPEG_URL) as response, open(zip_path, 'wb') as out_file:
            ...
            self.log("INFO", "Extracting binaries...")
            ...
            self.config.set("ffmpeg_path", DEFAULT_FFMPEG)
            self.config.set("ffprobe_path", DEFAULT_FFPROBE)
            self.log("INFO", "FFmpeg dependency installed successfully.")
            return True
        except Exception as e:
            self.log("ERROR", f"Failed to setup FFmpeg: {e}")
            try:
...
class AutoUpdater:
    ...
    def check_for_updates(self):
        if not self.is_frozen: return None
        try:
...
class FFmpegProcessor:
    ...
    def process_video(self, video_file, status_callback=None):
        ...
        ffmpeg_path = self.config.get("ffmpeg_path")
        cmd = [ffmpeg_path, "-y"]
        if self.config.get("gpu_acceleration") and self.config.get("hardware_decode"): cmd.extend(["-hwaccel", "cuda", "-hwaccel_output_format", "cuda"])
        cmd.extend(["-i", video_file.path])
        if self.config.get("gpu_acceleration"): encoder, preset = self.config.get("encoder"), self.config.get("encoder_preset")
        else: encoder, preset = "libx264", "medium"
        cmd.extend(["-c:v", encoder, "-preset", preset, "-b:v", f"{target_bitrate}k", "-maxrate", f"{int(target_bitrate * 1.5)}k", "-bufsize", f"{target_bitrate * 2}k"])
        if self.config.get("copy_audio"): cmd.extend(["-c:a", "copy"])
        else: cmd.extend(["-c:a", "aac", "-b:a", self.config.get("audio_bitrate")])
        cmd.extend(["-progress", self.progress_file, output_path])
        
        self.log("START", f"Processing: {video_file.filename}")
        try:
            self.current_process = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
            ...
            while self.current_process.poll() is None:
                ...
                if progress > 0:
                    ...
                    if self.progress_callback: self.progress_callback(video_file, progress)
                time.sleep(0.3)
    ...
    def read_progress_file(self, duration):
        try:
            ...
            if matches:
                ...
                if duration > 0: return min(99, int((current_sec / duration) * 100))
...
class QueueItem(ctk.CTkFrame):
    def __init__(self, parent, video_file, on_remove=None, **kwargs):
        ...
        ctk.CTkLabel(icon_frame, text="🎬", font=("Arial", 16)).place(relx=0.5, rely=0.5, anchor="center")
        
        info_frame = ctk.CTkFrame(self, fg_color="transparent")
        info_frame.grid(row=0, column=1, sticky="ew", pady=10)
        self.name_label = ctk.CTkLabel(info_frame, text=video_file.filename, font=("Arial", 12, "bold"), text_color="white", anchor="w")
        self.name_label.pack(fill="x")
        self.path_label = ctk.CTkLabel(info_frame, text=video_file.directory, font=("Arial", 10), text_color="#586a8a", anchor="w")
        self.path_label.pack(fill="x")
        
        self.size_label = ctk.CTkLabel(self, text=self.format_size(video_file.size), font=("Arial", 11), text_color="#92a4c9", width=80)
        self.size_label.grid(row=0, column=2, padx=10)
        self.status_label = ctk.CTkLabel(self, text=video_file.status.capitalize(), font=("Arial", 11, "bold"), text_color=self.get_status_color(), width=80)
        ...
        self.remove_btn = ctk.CTkButton(self, text="✕", width=30, height=30, fg_color="transparent", hover_color="#324467", text_color="#92a4c9", command=self.remove)
    ...
    def get_status_color(self):
        return {"pending": "#9ca3af", "processing": "#3b82f6", "completed": "#10b981", "error": "#ef4444", "skipped": "#f59e0b"}.get(self.video_file.status, "#9ca3af")
    
    def update_display(self):
        self.status_label.configure(text=self.video_file.status.capitalize(), text_color=self.get_status_color())
        self.progress_bar.set(self.video_file.progress / 100)
        if self.video_file.status == "completed": self.progress_bar.configure(progress_color="#10b981")
        elif self.video_file.status == "error": self.progress_bar.configure(progress_color="#ef4444")
        elif self.video_file.status == "processing": self.progress_bar.configure(progress_color="#135bec")
    
    def remove(self):
        if self.on_remove: self.on_remove(self.video_file)
...
class LogConsole(ctk.CTkFrame):
    def __init__(self, parent, **kwargs):
        ...
        for color in ["#ef4444", "#eab308", "#22c55e"]:
            ctk.CTkFrame(dots_frame, width=10, height=10, fg_color=color, corner_radius=5).pack(side="left", padx=2)
        ctk.CTkLabel(header, text="CONSOLE OUTPUT", font=("Consolas", 10, "bold"), text_color="#92a4c9").pack(side="left", padx=10)
        ctk.CTkButton(header, text="Clear", width=60, height=24, fg_color="#232f48", hover_color="#324467", font=("Arial", 10), command=self.clear).pack(side="right", padx=10, pady=8)
        self.log_text = ctk.CTkTextbox(self, font=("Consolas", 11), fg_color="#090c13", text_color="#92a4c9", wrap="word")
...
class SettingsPanel(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        title = ctk.CTkLabel(main_frame, text="Settings", font=("Arial", 28, "bold"), text_color="white")
        title.pack(anchor="w", pady=(0, 5))
        
        subtitle = ctk.CTkLabel(main_frame, text="Configure encoding, paths, and performance options.", font=("Arial", 12), text_color="#92a4c9")
        ...
        self.encoder_combo = ctk.CTkComboBox(enc_left, values=["h264_nvenc (NVIDIA GPU)", "hevc_nvenc (NVIDIA GPU)", "libx264 (CPU)", "libx265 (CPU)"], width=200, fg_color="#0d121c", border_color="#324467", button_color="#324467", dropdown_fg_color="#111722")
        ...
        self.preset_combo = ctk.CTkComboBox(enc_right, values=["p1 - Fastest", "p2 - Faster", "p3 - Fast", "p4 - Medium", "p5 - Slow", "p6 - Slower", "p7 - Best Quality"], width=200, fg_color="#0d121c", border_color="#324467", button_color="#324467", dropdown_fg_color="#111722")
        self.preset_combo.pack(anchor="w", pady=5)
        
        ctk.CTkFrame(encoding_frame, height=1, fg_color="#324467").pack(fill="x", padx=15)
        
        # Dual Bitrate Control Section
        ...
        self.pct_btn = ctk.CTkButton(mode_frame, text="Percentage", width=100, height=28, fg_color="#324467", hover_color="#324467", command=lambda: self.set_bitrate_mode("percentage"))
        ...
        self.pct_slider = ctk.CTkSlider(self.pct_frame, from_=5, to=100, command=lambda v: self.pct_value_label.configure(text=f"{int(v)}%"))
        ...
        self.size_slider = ctk.CTkSlider(self.size_frame, from_=1, to=100, command=lambda v: self.size_value_label.configure(text=f"{int(v)} MB"))
        ...
        self.audio_bitrate_combo = ctk.CTkComboBox(self.audio_bitrate_frame, values=["64k", "96k", "128k", "192k", "256k", "320k"], width=150, fg_color="#0d121c", border_color="#324467", button_color="#324467", dropdown_fg_color="#111722")
        self.audio_bitrate_combo.pack(anchor="w", pady=5)
        
        button_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
        button_frame.pack(fill="x", pady=20)
        ctk.CTkButton(button_frame, text="Reset Defaults", fg_color="#232f48", hover_color="#324467", width=120, command=self.reset_defaults).pack(side="left")
        ctk.CTkButton(button_frame, text="Save Changes", fg_color="#135bec", hover_color="#1d4ed8", width=120, command=self.save_settings).pack(side="right")
    ...
    def create_path_input(self, parent, label, config_key, is_folder=True):
        ...
        def browse():
            path = filedialog.askdirectory() if is_folder else filedialog.askopenfilename(filetypes=[("Executable", "*.exe"), ("All files", "*.*")])
            if path:
                entry.delete(0, "end")
                entry.insert(0, path)
        ctk.CTkButton(input_frame, text="Browse", width=70, fg_color="#232f48", hover_color="#324467", command=browse).pack(side="right", padx=(10, 0))
    ...
    def load_settings(self):
        ...
        else: self.hwdec_switch.deselect()
        
        encoder = self.config.get("encoder")
        encoder_map = {"h264_nvenc": "h264_nvenc (NVIDIA GPU)", "hevc_nvenc": "hevc_nvenc (NVIDIA GPU)", "libx264": "libx264 (CPU)", "libx265": "libx265 (CPU)"}
        self.encoder_combo.set(encoder_map.get(encoder, encoder_map["h264_nvenc"]))
        
        preset = self.config.get("encoder_preset")
        preset_map = {"p1": "p1 - Fastest", "p2": "p2 - Faster", "p3": "p3 - Fast", "p4": "p4 - Medium", "p5": "p5 - Slow", "p6": "p6 - Slower", "p7": "p7 - Best Quality"}
        ...
        if self.config.get("copy_audio"): self.copy_audio_switch.select()
        else: self.copy_audio_switch.deselect()
    ...
    def save_settings(self):
        ...
        enc_text = self.encoder_combo.get()
        if "h264" in enc_text: self.config.set("encoder", "h264_nvenc")
        elif "hevc" in enc_text: self.config.set("encoder", "hevc_nvenc")
        elif "x264" in enc_text: self.config.set("encoder", "libx264")
        elif "x265" in enc_text: self.config.set("encoder", "libx265")
        
        self.config.set("encoder_preset", self.preset_combo.get().split(" ")[0])
    ...
    def reset_defaults(self):
        for key, value in DEFAULT_CONFIG.items(): self.config.set(key, value)
        self.load_settings()
...
class VideoProcessorApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title(f"VideoProc - Video Processor {APP_VERSION}")
    ...
    def startup_sequence(self):
        ...
        dep_manager = DependencyManager(self.config, lambda l, m: self.after(0, lambda: self.console.log(l, m)))
        
        if not dep_manager.validate_installation():
            self.after(0, lambda: self.status_label.configure(text="Status: Installing FFmpeg..."))
            def install_prog(pct):
                self.after(0, lambda: self.overall_progress.set(pct/100))
            success = dep_manager.install_ffmpeg(install_prog)
            if not success:
                self.after(0, lambda: messagebox.showerror("Error", "Failed to setup FFmpeg. App may not function correctly."))
        
        self.after(0, lambda: self.console.log("INFO", "Application ready. Full GPU acceleration enabled!"))
        self.after(0, lambda: self.set_ui_state("unlocked"))
        self.after(0, self.check_resume)

    def set_ui_state(self, state):
        self.ui_locked = (state == "locked")
        if self.ui_locked:
            self.start_btn.configure(state="disabled")
            self.clear_btn.configure(state="disabled")
            self.status_label.configure(text="Status: Initializing Setup...")
        else:
    ...
    def check_resume(self):
        if not self.queue_state.exists(): return
        ...
        if messagebox.askyesno("Resume Queue", f"Found saved queue with {len(incomplete)} incomplete file(s).\n\nResume?"):
    ...
    def restore_queue(self, files):
        ...
        for f in files:
            ...
            video_file.relative_path = f.get("relative_path", "")
            
            base_name = Path(path).stem
            output_path = os.path.join(output_folder, video_file.relative_path, f"{base_name}{suffix}.mp4") if video_file.relative_path and video_file.relative_path != "." else os.path.join(output_folder, f"{base_name}{suffix}.mp4")
    ...
    def create_widgets(self):
        ...
        self.queue_frame = ctk.CTkScrollableFrame(content, fg_color="#111722", corner_radius=10)
        ...
        ctk.CTkLabel(empty_inner, text="Drop video files here", font=("Arial", 16, "bold"), text_color="#92a4c9").pack()
        ctk.CTkLabel(empty_inner, text="or click 'Add Files' to browse", font=("Arial", 12), text_color="#586a8a").pack(pady=5)
    ...
    def create_sidebar(self, parent):
        ...
        for icon, text, command, active in nav_buttons:
            ctk.CTkButton(nav_frame, text=f"  {icon}  {text}", anchor="w", fg_color="#232f48" if active else "transparent", hover_color="#232f48", text_color="white" if active else "#92a4c9", height=40, command=command).pack(fill="x", pady=2)
        ...
        self.status_label = ctk.CTkLabel(progress_info, text="Status: Idle", font=("Arial", 11), text_color="#92a4c9")
        ...
        self.start_btn = ctk.CTkButton(controls_frame, text="▶  Start Processing", fg_color="#135bec", hover_color="#1d4ed8", height=45, font=("Arial", 13, "bold"), command=self.start_processing)
        self.start_btn.pack(fill="x", padx=15, pady=(0, 10))
        
        self.stop_btn = ctk.CTkButton(controls_frame, text="⏹  Stop Processing", fg_color="#dc2626", hover_color="#b91c1c", height=45, font=("Arial", 13, "bold"), command=self.stop_processing)
        
        self.clear_btn = ctk.CTkButton(controls_frame, text="Clear Queue", fg_color="#232f48", hover_color="#324467", height=35, command=self.clear_queue)
    ...
    def create_header(self, parent):
        ...
        ctk.CTkLabel(title_frame, text="Video Queue", font=("Arial", 28, "bold"), text_color="white").pack(anchor="w")
        
        ctk.CTkButton(header, text="➕  Add Files", fg_color="white", text_color="black", hover_color="#e5e5e5", height=40, width=120, font=("Arial", 12, "bold"), command=self.add_files).pack(side="right", padx=20, pady=30)
        ctk.CTkButton(header, text="📁  Add Folder", fg_color="#232f48", text_color="white", hover_color="#324467", height=40, width=120, font=("Arial", 12, "bold"), command=self.add_folder).pack(side="right", padx=5, pady=30)
    ...
    def create_stats_bar(self, parent):
        ...
        for icon, title, attr, default in stats:
            ...
            ctk.CTkLabel(inner, text=f"{icon}  {title}", font=("Arial", 10, "bold"), text_color="#92a4c9").pack(anchor="w")
            label = ctk.CTkLabel(inner, text=default, font=("Arial", 22, "bold"), text_color="white")
            label.pack(anchor="w")
            setattr(self, attr, label)
    
    def add_files(self):
        if self.ui_locked: return
        files = filedialog.askopenfilenames(filetypes=[("Video files", "*.mp4 *.mkv *.mov *.avi *.wmv *.flv *.webm"), ("All files", "*.*")])
        ...
        for root, dirs, filenames in os.walk(folder):
            for f in filenames:
                if Path(f).suffix.lower() in extensions:
                    files_with_paths.append((os.path.join(root, f), os.path.relpath(root, base_folder_parent)))
        if files_with_paths: self.add_to_queue_with_structure(files_with_paths)
        else: messagebox.showinfo("Info", "No video files found")
    ...
    def add_to_queue_with_structure(self, files_with_paths):
        ...
        for path, rel_path in files_with_paths:
            ...
            video_file = VideoFile(path)
            video_file.relative_path = rel_path
    ...
    def add_to_queue(self, file_paths):
        ...
        for path in file_paths:
            ...
            info = processor.get_video_info(path)
            if info:
                video_file.size, video_file.duration, video_file.bitrate, video_file.video_bitrate = info["size"], info["duration"], info["bitrate"], info["video_bitrate"]
            else:
                video_file.status, video_file.error = "error", "Could not read file info"
            self.queue.append(video_file)
            self.add_queue_item(video_file)
        self.update_stats()
    ...
    def add_queue_item(self, video_file):
        ...
        item = QueueItem(self.queue_frame, video_file, on_remove=self.remove_from_queue)
        item.pack(fill="x", pady=5, padx=5)
        self.queue_items[video_file] = item
    
    def remove_from_queue(self, video_file):
        if video_file.status == "processing": return
        if video_file in self.queue: self.queue.remove(video_file)
    ...
    def rescan_queue(self):
        if not self.queue: return messagebox.showinfo("Info", "Queue is empty")
        output_folder, suffix, updated, reset = self.config.get("output_folder"), self.config.get("filename_suffix"), 0, 0
        for vf in self.queue:
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                if vf.status != "completed": vf.status, vf.progress, updated = "completed", 100, updated + 1
            else:
                if vf.status == "completed": vf.status, vf.progress, reset = "pending", 0, reset + 1
    ...
    def update_stats(self):
        total, pending, completed = len(self.queue), len([vf for vf in self.queue if vf.status == "pending"]), len([vf for vf in self.queue if vf.status == "completed"])
        ...
        self.stat_size.configure(text=self.queue_items[list(self.queue_items.keys())[0]].format_size(total_size) if self.queue else "0 MB")
        if total > 0: self.overall_progress.set(completed / total)
    ...
    def start_processing(self):
        ...
        if skipped > 0: self.console.log("INFO", f"Skipped {skipped} already completed file(s)")
        self.update_stats()
        
        pending = [vf for vf in self.queue if vf.status == "pending"]
        if not pending: return messagebox.showinfo("Info", "No pending files to process.\n\nAll files are already completed!")
    ...
    def process_queue(self):
        ...
        for video_file in self.queue:
            ...
            if video_file.status != "pending": continue
        ...
        if len([vf for vf in self.queue if vf.status == "pending"]) == 0: self.queue_state.clear()
    ...
    def update_queue_item(self, video_file):
        if video_file in self.queue_items: self.queue_items[video_file].update_display()
    ...
    def open_settings(self):
        if not self.ui_locked: SettingsPanel(self, self.config, on_save=lambda: self.console.log("INFO", "Settings saved"))
    ...
    def open_output_folder(self):
        ...
        except Exception as e: messagebox.showwarning("Warning", f"Cannot open folder:\n{e}")
